...
Spec = Gem::Specification.new do |s|
  ...
  s.require_paths = ['lib','doc/lib']

  s.has_rdoc = true

  s.rdoc_options << PKG::RDOC_OPTIONS 
  #s.rdoc_options << ["--format=riofish"] 
end
...
task :upload_rio4ruby_doc => [:rdoc] do
  puts "Creating gzip file"
  sh "tar czf doc.tar.gz doc;"
  sh "scp -P 7822 doc.tar.gz kleckner@rio4ruby.com:www/;"
  sh "ssh -p 7822 kleckner@rio4ruby.com 'cd www;tar xzf doc.tar.gz'"

end

desc "Upload documentation to homepage"
task :upload_rubyforge_doc => [:rdoc] do
  Dir.chdir('doc')
  puts
