...
class RDoc::Generator::Parts
	...
	def initialize( options )
		...
		template = @options.template || 'parts'
	...
	### Generate an index page which lists all the classes which
	### are documented.
	def generate_parts_classes
		template_file = @template_dir + 'parts-classes.rhtml'
		return unless template_file.exist?

		debug_msg "Rendering the parts-classes page..."
		...
		outfile = @basedir + @options.op_dir + 'parts-classes.html'
		unless $RIOFISH_DRYRUN
			debug_msg "Outputting to %s" % [outfile.expand_path]
			outfile.open( 'w', 0644 ) do |fh|
				fh.print( output )
	...
	### Generate an index page which lists all the classes which
	### are documented.
	def generate_parts_files
		template_file = @template_dir + 'parts-files.rhtml'
		return unless template_file.exist?

		debug_msg "Rendering the parts-files page..."
		...
		outfile = @basedir + @options.op_dir + 'parts-files.html'
		unless $RIOFISH_DRYRUN
			debug_msg "Outputting to %s" % [outfile.expand_path]
			outfile.open( 'w', 0644 ) do |fh|
				fh.print( output )
  ...
  def part_path(pth,partname)
    en = File.extname(pth)
    bn = File.basename(pth,en)
    dn = File.dirname(pth)
    File.join(dn,bn + '-part-' + partname + en)
  end
	### Generate a documentation file for each class
	def generate_class_parts(partname)
		template_file = @template_dir + ('part-class-' + partname + '.rhtml')
		debug_msg "gen class_parts with #{template_file}"
		return unless template_file.exist?
		debug_msg "Generating class part #{partname} in #@outputdir"

		@classes.each do |klass|
			debug_msg "  working on %s (%s)" % [ klass.full_name, klass.path ]
			#p "  working on %s (%s)" % [ klass.full_name, klass.path ]
			outfile    = @outputdir + part_path(klass.path,partname)
